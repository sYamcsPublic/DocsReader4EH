...
function getReadingPositions(): Record<string, number> {
  return JSON.parse(localStorage.getItem('g_reading_positions') || '{}');
}
...
export class ReaderPage extends BasePage {
  ...
  constructor(
    ...
    // Always get the latest positions from localStorage
    const positions = getReadingPositions();
    const storedVal = positions[file.id] || 0;
    // Heuristic: if value is > 1.1, it's likely an old page index (0, 1, 2...). Ratios are 0-1.
    // We allow up to 1.1 just in case of float rounding, but exactly 1.0 is 100%.
    const ratio = (storedVal > 1.1) ? 0 : storedVal;
    ...
    // Reset if we are at the end and the flag is set
    if (resetIfAtEnd && this.currentPageIndex >= this.pages.length - 1) {
  ...
  async updateContent() {
    ...
    // Refresh battery (throttled in manager) to keep everything in sync
